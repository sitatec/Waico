...
class _ExercisePageState extends State<ExercisePage> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  Future<void> _initializeWorkoutSession() async {
    ...
    try {
      // With the camera stream, VAD, real-time pose landmark detection, exercise classification, audio recording,
      // speech to text,  LLM inference, and text to speech all working together is computationally heavy, causing latencies
      // in the feedback messages on Samsung S21 Ultra. This may not be an issue on higher-end devices like the S24/25 Ultra,
      // or maybe there are some optimizations we can do to address this. But for now, we disabled voice chat during workout
      // sessions and use pre-recorded feedback messages. The system still works well and provides appropriate feedback,
      // randomly picking 1 from 5 messages for each form correction type to make it feel a bit natural, but still not as
      // good as when generated by an LLM that has full context and past reps data in the chat history.
      // You can enable voice chat by setting `enableVoiceChat` to true below.
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (exercise.load.type == ExerciseLoadType.duration &&
                ...
                Container(
                  ...
                  child: Text(
                    // 'No reps counting for cardio exercises like ${exercise.name}. Focus on maintaining a steady pace and form.',
                    LocaleKeys.workout_exercise_no_reps_counting.tr(namedArgs: {'exerciseName': exercise.name}),
                    style: TextStyle(color: Colors.amber, fontSize: 14, fontWeight: FontWeight.w500),
                  ),
                ),
              ] else if (exercise.optimalView != null) ...[
                const SizedBox(height: 16),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                  decoration: BoxDecoration(
